...
# Check if a VPC endpoint for ECR already exists
def find_existing_vpc_endpoint():
    response = ec2.describe_vpc_endpoints(
        Filters=[
            {'Name': 'vpc-id', 'Values': [vpc_id]},
            {'Name': 'service-name', 'Values': [f'com.amazonaws.{region}.ecr.dkr']}
        ]
    )
    return response['VpcEndpoints']

existing_endpoints = find_existing_vpc_endpoint()

if existing_endpoints:
    vpc_endpoint_id = existing_endpoints[0]['VpcEndpointId']
    print(f"Using existing VPC Endpoint: {vpc_endpoint_id}")
else:
    # Create a VPC endpoint for ECR in the specific subnet
...
def push_image():
    ...
    for i in range(retries):
        ...
        if push_status == 0:
            return
        print("Re-authenticating and retrying push...")
        ecr_login()
        time.sleep(5)
    raise Exception("Failed to push the Docker image to ECR after multiple attempts")
